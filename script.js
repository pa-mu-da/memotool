    ...
    div.innerHTML = `
      <div class="char-info">
        ...
        <div class="char-info-content">
          ...
          ${char.note !== undefined ? `<div class="char-note">${char.note}</div>` : '<div class="char-note"></div>'}
          <button class="copy-button" data-json="${encodeURIComponent(char.json)}" onclick="copyJson(this)">駒をコピー</button>
        </div>
      </div>
...
function exportHtmlFullPage() {
  ...
  urlList.querySelectorAll('button').forEach(btn => btn.remove());
  charList.querySelectorAll('button.remove-button').forEach(btn => btn.remove());
